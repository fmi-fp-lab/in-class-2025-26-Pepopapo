...
-- TODO:
-- Either (Maybe, but for exceptions instead of just nullability)
-- abstracting/automating recursion (look at last week's solutions)
-- - show how we reached map (for lists)
-- - squareList
-- - megaPair
-- folds as "replacing constructors"
-- briefly mention unfolds, {cata,ana}morphisms
...
-- (Succ $ Succ $ Succ Zero)
--
-- foldNat 0 (1+) (Succ $ Succ $ Succ Zero)
-- (1+) $ foldNat 0 (1+) (Succ $ Succ Zero)
-- (1+) $ (1+) $ foldNat 0 (1+) (Succ Zero)
-- (1+) $ (1+) $ (1+) $ foldNat 0 (1+) Zero
-- (1+) $ (1+) $ (1+) 0
...
-- TODO: implement multNat (for expNat)
...
foldr :: b -> (a -> b -> b) -> [a] -> b
foldr nv _ [] = nv
foldr nv op (x : xs) = op x (foldr nv op xs)
