...
-- TODO:
-- Lists
-- - ask about linked list in C/C++, explain what they are
-- - show definition, contrast with builtin (again, hoogle workflow)
-- - data List
-- - sumList, replicate
-- - explain naming convention (akin to scheme/lisp in general)
-- - list comprehension (!!!) (generators, `let`s, filters, compare with set builder notation)
-- - cartesianProd with list comprehension?
-- - brief mention of Strings (C mentioned??????) (for performance, Text exists)

-- data Лист a = Кутийка a (Лист a) | Нищо
-- data List a = Cons a (List a) | Nil
-- data Banica a = a :------------ (Banica a) | Banica
-- data [] a = (:) a ([] a) | []
--
-- infixr 7 :------------
--
-- елемент :: Лист Bool
-- елемент = Кутийка True $ Кутийка False Нищо
--
-- елемент2 :: Banica Bool
-- елемент2 = True :------------ False :------------ Banica
...
ys :: [Integer]
ys = [ x | x <- [-10..10], y <- [1.2,2.2,3.2], let z = 6 ]

-- >>> ys
-- [-1.9,-1.9,-1.9,1.2,1.2,1.2,0.2,0.2,0.2,3.2,3.2,3.2]

-- TODO:
-- Maybe
-- - explicit permission for a value to be missing
-- - usually called nullability (Kotlin, Java, C#) (why not js?)
-- - show definition
...
-- TODO:
-- try doing some exercises with recursion and some with list comprehension (or both)
...
cartesian :: [a] -> [b] -> [(a, b)]
cartesian xs bs = concat $ map  (\ x -> megaPair x bs) xs
...
lift2List f lst1 lst2 = [f x y | x <- lst1 , y<-lst2]
...
validateList (Just x: xs) =
  case validateList xs of
    Nothing -> Nothing
    Just listOfXs -> Just (x:listOfXs)
...
digitsToNumber xs = fromIntegral (go (reverse $ map fromEnum xs) 0)
  where
    -- for some reason, we often call helpers in haskell "go", as in "go do the thing"

    go (l:lst) x =l*(10 ^ x) + go lst (x+1)
    go [] _ = 0
...
parseNumber xs
  |isNothing $ validateList ( map parseDigit xs)  = Nothing
  |otherwise                                   =  Just $ digitsToNumber $ go ( map parseDigit xs)
    where
      go (Just l: lst) = l :go lst
      go (Nothing : _) = []
      go [] = []
...
reverseLinear :: [a] -> [a]
reverseLinear xs = go xs []
  where
    go (b:bs) rs = go bs (b: rs)
    go [] rs  = rs
